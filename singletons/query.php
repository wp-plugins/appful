...
class Appful_API_Query {
    ...
    function get_legacy_controller($json) {
        ...
        if ($json == 'submit_comment') {
            if ($appful_api->controller_is_active('respond')) {
                return 'respond';
            } else {
                $appful_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > JSON API.");
            }
        } else if ($json == 'create_post') {
            if ($appful_api->controller_is_active('posts')) {
                return 'posts';
            } else {
                $appful_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API.");
            }
        } else {
            return 'core';
        }
    ...
    // Note:
        ...
        if (empty($method)) {
            ...
            return false;
        } else if (method_exists("Appful_API_{$controller}_Controller", $method)) {
