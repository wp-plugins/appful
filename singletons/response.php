...
class Appful_API_Response {

	function setup() {
		global $appful_api;
		$this->include_values = array();
		$this->exclude_values = array();
		if ($appful_api->query->include) {
			$this->include_values = explode(',', $appful_api->query->include);
		}
		// Props to ikesyo for submitting a fix!
		if ($appful_api->query->exclude) {
			$this->exclude_values = explode(',', $appful_api->query->exclude);
	...
	function get_json($data, $status = 'ok') {
		...
		$data = apply_filters('appful_api_encode', $data);
	...
	function respond($result, $status = 'ok') {
		...
		if ($appful_api->query->dev || !empty($_REQUEST['dev'])) {
			...
			} else if ($appful_api->query->redirect) {
				$url = $this->add_status_query_var($appful_api->query->redirect, $status);
				wp_redirect($url);
			} else if ($appful_api->query->callback) {
				// Run a JSONP-style callback with the result
				$this->callback($appful_api->query->callback, $json);
			} else {
			// Output the result
			$this->output($json);
		}
