...
class Appful_API_Post {
    ...
    function set_content_value() {
        ...
        if ($appful_api->include_value('content')) {
            $content = get_the_content($appful_api->query->read_more);


            preg_match_all("/\[.*?\]/i", $content, $matches);
            foreach ($matches[0] as $match) {
                if (strpos($match, "[poll") !== FALSE) {
                    $content = str_replace($match, "<iframe class=\"wp-poll\" frameborder=\"0\" scrolling=\"no\" id=\"iframe\" onload=\"resizeIframe(this);\" src=\"". get_option("siteurl", "") ."/wp-content/plugins/appful/plugins/wp-polls.php?id=" . explode("\"", $match)[1] . "\"></iframe>", $content);
        ...
        if ($appful_api->include_value('tags')) {
            ...
            if ($wp_tags = get_the_tags($this->id)) {
                foreach ($wp_tags as $wp_tag) {
                    $this->tags[] = new Appful_API_Tag($wp_tag);
                }
            }
        } else {
            unset($this->tags);
    ...
    function set_thumbnail_value() {
        global $appful_api;
        if (!$appful_api->include_value('thumbnail') ||
